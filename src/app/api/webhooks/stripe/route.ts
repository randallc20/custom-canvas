  ...
  switch (event.type) {
    ...
    case 'charge.dispute.updated': {
      ...
      const { data: order, error: orderReadError } = await supabase
        .from('orders')
        .select('id, status, artist_id, stripe_refund_id, dispute_id, dispute_status, dispute_outcome, signature_required, signature_confirmed, listing:listings(title)')
        .eq('stripe_payment_intent_id', paymentIntentId)
        .maybeSingle();
      if (orderReadError) {
      ...
      // Requirement 4 is ours to satisfy, not the artist's (ruling D7), and
      // the verdict above is frozen the instant this webhook lands — long
      // before anyone could open a tracking page. Tell the admin to go and
      // look; recording it on /admin/orders re-assesses the order.
      const signatureTodo =
        !covered && order.signature_required && !order.signature_confirmed
          ? ` This order is $750 or more and signature confirmation is NOT recorded — open the carrier's tracking page, and if a signature event exists, record it on /admin/orders BEFORE responding. That re-assesses the order.`
          : '';
      for (const adminId of admins) {
        rows.push({
          user_id: adminId,
          type: 'order_disputed',
          title: 'Payment disputed',
          body: `A dispute was opened on "${title}" — protection: ${protectionStatus}. Respond in the Stripe dashboard before the bank's deadline.${signatureTodo}`,
          link: '/admin/orders',
        });
      }
      if (rows.length) await supabase.from('notifications').insert(rows);
